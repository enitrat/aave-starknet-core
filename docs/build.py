...
def create_dirs():
    # get the directory full path from the file path
    directory = path.parent
    # remove `/data` from directory
    directory = str(directory).replace("/data", "")
...
class Document():
    ...
    def create_api_page(self):
        ...
        filepath = self._root/self._name
        if not os.path.exists(self._root):
            os.makedirs(self._root)
        markdown = MdUtils(file_name=filepath.as_posix(),
                           title=self._name.capitalize())
